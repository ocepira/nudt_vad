...
def format_res_gt_by_classes(result_path,
    ...
    # print('Cls data formatting done in {:2f}s!! with {}'.format(float(timer.since_start()),formatting_file))  #屏蔽
    pool.close()
    return cls_gens, cls_gts
...
def eval_map(gen_results,
    ...
    for i, clsname in enumerate(cls_names):
        ...
        # print('cls:{} done in {:2f}s!!'.format(clsname,float(timer.since_last_check())))  #屏蔽
    pool.close()
    aps = []
    ...
    # print_map_summary(
        # mean_ap, eval_results, class_name=cls_names, logger=logger)   #屏蔽
